...
# Install dependencies with specific versions for Windows 7 compatibility
...
function Install-Package {
    param($PackageName, $Version = "", $DevDependency = $false)
    
    $installCmd = "npm install"
    if ($DevDependency) { $installCmd += " --save-dev" }
    if ($Version) { $installCmd += " $PackageName@$Version" } else { $installCmd += " $PackageName" }
    ...
    try {
        ...
        try {
            Invoke-Expression "$installCmd --force"
            Write-Host "✅ $PackageName installed (forced)" -ForegroundColor Green
        } catch {
...
Write-Host "`n🔧 Installing core Electron and React dependencies..." -ForegroundColor Cyan
Install-Package "electron" "21.4.4" $true
Install-Package "react" "18.2.0"
Install-Package "react-dom" "18.2.0"

# Install TypeScript and build tools
Write-Host "`n🔨 Installing build tools..." -ForegroundColor Cyan
Install-Package "@types/react" "18.2.0" $true
Install-Package "@types/react-dom" "18.2.0" $true
Install-Package "@types/node" "16.18.0" $true
Install-Package "typescript" "4.9.5" $true
Install-Package "vite" "4.2.0" $true
Install-Package "@vitejs/plugin-react" "3.1.0" $true
Install-Package "electron-builder" "24.6.0" $true
Install-Package "concurrently" "7.6.0" $true

# Install UI and styling dependencies
Write-Host "`n🎨 Installing UI and styling dependencies..." -ForegroundColor Cyan
Install-Package "tailwind-merge" "1.10.0"
Install-Package "clsx" "1.2.1"
Install-Package "tailwindcss" "3.3.0" $true
Install-Package "autoprefixer" "10.4.16" $true
Install-Package "postcss" "8.4.31" $true
Install-Package "framer-motion" "10.8.0"
Install-Package "lucide-react" "0.323.0"
Install-Package "react-hot-toast" "2.4.0"

# Install routing and i18n
Write-Host "`n🌐 Installing routing and internationalization..." -ForegroundColor Cyan
Install-Package "react-router-dom" "6.8.1"
Install-Package "react-i18next" "12.2.0"
Install-Package "i18next" "22.4.13"
Install-Package "i18next-browser-languagedetector" "7.0.1"
Install-Package "i18next-http-backend" "2.2.0"

# Install database dependencies (Windows 7 compatible)
Write-Host "`n💾 Installing database dependencies..." -ForegroundColor Cyan
Install-Package "better-sqlite3" "8.7.0"
Install-Package "kysely" "0.25.0"

# Install hardware integration
Write-Host "`n🔌 Installing hardware integration..." -ForegroundColor Cyan
Install-Package "node-machine-id" "1.1.12"

# Install analytics and state management
Write-Host "`n📊 Installing analytics and state management..." -ForegroundColor Cyan
Install-Package "recharts" "2.5.0"
Install-Package "zustand" "4.3.6"
Install-Package "electron-store" "8.1.0"

# Install testing framework
Write-Host "`n🧪 Installing testing framework..." -ForegroundColor Cyan
Install-Package "jest" "28.1.3" $true
Install-Package "@testing-library/react" "13.4.0" $true
Install-Package "@testing-library/jest-dom" "5.17.0" $true
Install-Package "ts-jest" "28.0.8" $true
Install-Package "@testing-library/user-event" "14.5.0" $true
Install-Package "jest-environment-jsdom" "28.1.3" $true
Install-Package "@playwright/test" "1.40.0" $true

# Install linting and code quality
Write-Host "`n🔍 Installing linting and code quality tools..." -ForegroundColor Cyan
Install-Package "eslint" "8.50.0" $true
Install-Package "@typescript-eslint/eslint-plugin" "6.7.0" $true
Install-Package "@typescript-eslint/parser" "6.7.0" $true

# Try bulk install as fallback
Write-Host "`n📦 Running bulk dependency installation..." -ForegroundColor Yellow
try {
    npm install --no-optional --legacy-peer-deps
    Write-Host "✅ Bulk installation completed" -ForegroundColor Green
} catch {
    Write-Host "⚠️ Bulk installation had issues, but individual packages were installed" -ForegroundColor Yellow
}
...
try {
    ...
    $devShortcut.IconLocation = "$PWD\build\icon.ico"
    ...
    $prodShortcut.IconLocation = "$PWD\build\icon.ico"
